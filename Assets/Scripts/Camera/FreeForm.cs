    ...
    public class FreeForm : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            if (platform.Contains("windows") || platform.Contains("linux"))
            {
                ...
                rightAxisXName = "RightStickH";
                rightAxisYName = "RightStickV";
            }
            // else
            // {
            //     rightAxisXName = "RightV";
            //     rightAxisYName = "RightH";
            ...
            {
                Debug.LogWarning("Controller Error - Right axis not set in InputManager. Joystick is disabled!");
        ...
        public void Update()
        {
            if (cameraEnabled)
            {
                ...
                if (Input.GetAxis("Mouse ScrollWheel") < 0) // back
                {
                    ...
                    if(cameraController.desiredDistance > maxZoomDistance)
                        cameraController.desiredDistance = maxZoomDistance;
                }
                if (Input.GetAxis("Mouse ScrollWheel") > 0) // forward
                {
                    cameraController.desiredDistance -= cameraController.zoomOutStepValue;
                    if(cameraController.desiredDistance < minZoomDistance)
                        cameraController.desiredDistance = minZoomDistance;
                }
